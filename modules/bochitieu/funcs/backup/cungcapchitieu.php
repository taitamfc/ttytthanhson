...
if (empty($user_info)){	$url = MODULE_LINK . '&' . NV_OP_VARIABLE . '=login';nv_redirect_location($url); exit();}
	...
	if ($sta=='setdiemkehoach')
	{
		...
		$k=$key[2].'_'.$key[3].'_'.$key[1];//.'_'.$key[3];
	...
	if ($sta=='updateghichu')
	{
		...
		$k=$key[1].'_'.$key[2].'_'.$key[3];//.'_'.$key[3];
		$ghichu=$nv_Request->get_title('ghichu', 'get,post', '');
		if (!empty($ghichu)){
		$sql = 'Update ' . TABLE. '_ketqua_'.$key[1]. " SET ghichu='".$ghichu."'  WHERE dinhdanh ='".$k."'";
		$stmt = $db->prepare($sql);
		$row_id=$stmt->execute();
	...
	if ($sta=='getvalue')
	{
		...
		$namapdung=$key[2]; 
		$dinhdanh=$key[2].'_'.$key[3].'_'.$key[1];//năm_nhóm câu hỏi_ tiểu mục
		...
		if (!empty($kq))
		{
			$bv_danhgia=explode(";",$kq['bv_danhgia']);
			$doan_danhgia=explode(";",$kq['doan_danhgia']);
			$xtpl->assign('KQ', $kq);
		}
		...
		$token=md5($user_info['username'].$listid);
	...
	if ($sta=='savedanhgia')
	{
		...
		$diem=($key[6]>0)?$key[6]:0;
		if (($key[5]==1))
			$update="bv_danhgia='".$key[0]."', diem_bvdg=".$diem;
		else
			$update="doan_danhgia='".$key[0]."', diem_doandg=".$diem;
		
		$dinhdanh=$key[2].'_'.$key[3].'_'.$key[4];
		$desc=$user_info['username']." update ".date('d/m/Y h:i');
		$sql = 'Update ' . TABLE. '_ketqua_'.$key[2]. " 
		SET ".$update." , description='".$desc."'  WHERE dinhdanh ='".$dinhdanh."'";
		$stmt = $db->prepare($sql);
		$row_id=$stmt->execute();
		if ($row_id>0) $ketqua='OK_'.$lang_module['update_ok'];
		else $ketqua='ERR_'.$lang_module['update_err'];
	...
	if ($sta=='saveimage')
	{
		...
		if ($uploadOk == 1)
		{
			...
			else 
			{
				...
				$stmt->bindParam(':token', $token, PDO::PARAM_STR);
				$stmt->bindParam(':account', $user_info['username'], PDO::PARAM_STR);
				$stmt->bindParam(':namapdung', $key[1], PDO::PARAM_STR);
	...
	if ($sta=='copydanhgia')
	{
		...
		$sql = 'Update ' . TABLE. '_ketqua_'.$key[2]. " 
		SET doan_danhgia=bv_danhgia, diem_doandg=diem_bvdg , description='".$desc."'  WHERE token ='".$key[1]."'";
		$stmt = $db->prepare($sql);
		$row_id=$stmt->execute();
		if ($row_id>0) $ketqua='OK_'.$lang_module['update_ok'];
		else $ketqua='ERR_'.$lang_module['update_err'];
	...
	$log=substr($key[1],0,4)-1;$log1=substr($key[1],0,4)-2;
	$xtpl->assign('log', 'Năm '.$log);
	$xtpl->assign('log1', 'Năm '.$log1);
	//check log $log=$log.'12'; 
	$check_log=false;
	$sql = 'SELECT * FROM ' . TABLE. "_apdung where nam='".$log."12'";	
	...
	for ($i=1;$i<=$solan;$i++)
	{
		...
		$kq=array();
		$sql = 'SELECT * FROM ' .TABLE. "_ketqua_".$key[1]." where status=1 and token like '".md5($code.$user_info['username'])."'";
		...
		*/
		...
		foreach($list as $cs)
		{
			...
			foreach($Q as $q)
			{
				$q['token']=md5($q['code']).'_'.$q['code'].'_'.$key[1].'_'.$key[2] ;//.$user_info['username']
				$xtpl->assign('Q', $q);	
				
				$kq=array(); 
				$sql = 'SELECT * FROM ' .TABLE. "_ketqua_".$key[1]." where status=1 
				and dinhdanh like '".$key[1].'_'.$key[2].'_'.$q['code']."'";
				...
				if ($check_log)
				{
					$sql = 'SELECT * FROM ' .TABLE. "_ketqua_".$log."12 where status=1 
					and dinhdanh like '".$log."12_".$key[2].'_'.$q['code']."'";
