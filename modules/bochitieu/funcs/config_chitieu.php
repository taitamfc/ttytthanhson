...
if (empty($user_info)){	$url = MODULE_LINK . '&' . NV_OP_VARIABLE . '=login';nv_redirect_location($url); exit();}
	...
	if ($sta=='default_item_month')
	{
		$token=$nv_Request->get_title('token', 'get,post', '');
		$key=explode('_',$token);
		if ($key[0]!=md5($client_info['session_id'] . $key[2])) die('Stop!!!');
		$stmt = $db->prepare("Update ".TABLE."_apdung set thangapdung=".$key[2]." where nam=".$key[1])->execute();
		//$sql = 'Update '.TABLE.'_apdung set apdung=1 where nam='.$key[1]; 
		$stmt = $db->prepare($sql);
		$row_id=$stmt->execute();
		if ($row_id>0) $ketqua='OK_'.$row_id;
