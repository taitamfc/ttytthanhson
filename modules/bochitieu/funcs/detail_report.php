...
if (empty($user_info)){	$url = MODULE_LINK . '&' . NV_OP_VARIABLE . '=login';Header('Location: ' . nv_url_rewrite($url, true)); exit();}
	...
	$select_month=$apdung['thangapdung'];
	$xtpl->assign('BC', $apdung);
		for ($i=1;$i<=12;$i++)
		{
			...
			$item['token']=md5($client_info['session_id'] .$i).'_'.$apdung['thangapdung'].'_'.$i;
			if ($i==$apdung ['thangapdung']) {$item['color']='';$item['cls_div']='btn-primary';}
