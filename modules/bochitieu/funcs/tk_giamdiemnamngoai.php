...
if (empty($user_info)){	$url = MODULE_LINK . '&' . NV_OP_VARIABLE . '=login';Header('Location: ' . nv_url_rewrite($url, true)); exit();}
	...
	foreach ($list_chitieu as $r)
	{
		...
		foreach($Q as $q)
		{
			...
			$r['sl_chitieu']++;
			//show log
				if((!$check_doandg and $kq['diem_bvdg']<$kq['diem_namngoai']) 
					or ($check_doandg and $kq['diem_doandg']<$kq['diem_namngoai']) )
				$xtpl->parse('main.chitieu.loop');
		}
		//$Ch['value'].=$r['sl_chitieu'].',';
		//$Ch['label'].="'".$r['chi_so']."(".$r['sl_chitieu'].")',";
		$xtpl->assign('R', $r);
	...
	//Show Content
	/*$sql = 'SELECT * FROM ' . TABLE. '_baocao_'.$apdung['nam'].' where thang='.$select_month.' and nam='.$apdung['nam'];
