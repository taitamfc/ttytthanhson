...
$specialistName[] = $lang_module['selectspecialist'];
    ...
    foreach ($array_department as $k => $department) {
        if ($department['alias'] == $alias_url) {
		   ...
		   $time_book = array();
			...
			$specialist = array();
            $specialistName = array_map('trim', explode('|', $department['specialist']));
            foreach ($specialistName as $_specialist2) {
...
if ($nv_Request->isset_request('checkss', 'post')) {
	...
	$fspecialist = $nv_Request->get_int('fspecialist', 'post', 0);
	if (isset($specialist[$fspecialist])) {
        $fspecialist = $specialist[$fspecialist][1];
    } 
	else {

        $fspecialist = $specialist[0][1];
    }
    ...
    (cid, time_book, specialist,doctor, title, content,days, send_time, sender_id, sender_name,sender_birthday, sender_email, sender_phone, sender_address, sender_ip, is_read, is_reply) VALUES
    (' . $fpart . ', :time_book,:specialist,:doctor, :title, :content,:days, ' . NV_CURRENTTIME . ', ' . $sender_id . ', :sender_name,:sender_birthday, :sender_email, :sender_phone, :sender_address, :sender_ip, 0, 0)';
    $data_insert = array();
    $data_insert['time_book'] = $ftime_book;
	$data_insert['specialist'] = $fspecialist;
    ...
    if ($row_id > 0) {
        $fcon_mail = contact_sendcontact($row_id, $ftime_book, $fspecialist,$fdoctor, $ftitle, $fname,$fbirthday,$fdays, $femail, $fphone, $fcon, $fpart);
        ...
        if ($fsendcopy) {
            ...
            $fcon_mail = contact_sendcontact($row_id, $ftime_book,$fspecialist,$fdoctor, $ftitle, $fname, $fbirthday,$fdays, $fname, $femail, $fphone, $fcon, $fpart, false);
...
$contents = contact_main_theme($array_content, $array_department, $time_bookName,$specialistName, $doctorName, $base_url, NV_CHECK_SESSION);
