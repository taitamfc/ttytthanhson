...
if (!empty($get['acc']) && !empty($get['count_report'])) {
    ...
    $xtpl->assign('id_rp_detail', $id_report);
    ...
    */
    ...
    if ($update) {
        ...
        VALUES (:images ,:evaluation_type,:arr_question_type,:arr_result_question,:total_score, :rating,:arr_team, :from_date,:to_date,:created_date,
            :arr_question, :recommendation, :bonus, :discipline, :m0, :m1, :m2, :limitation,:count,:account_report,:id_report,:name_report,:account_input,:content,:status
        )";
        //         print($st);
        $stmt = $db->prepare($st);
        $stmt->bindParam(':evaluation_type', $post['evaluation_type']);
