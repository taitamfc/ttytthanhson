...
if ($nv_Request->get_title('act', 'get,post', '') == 'get_khoaphong_rand') {
    ...
    foreach ($rows as $r) {
        ...
        $item['y'] = $r['count'];
        $item['dtb'] = round($r['total_score'] / 50, 2);
        $item['ngay'] = $r['created_date'];
        $item['xl'] = $r['rating'];
        $item['KP'] = $kp;
        $item['RAND_KP'] = $kp;
...
if ($nv_Request->get_title('act', 'get,post', '') == 'get_dskhoaphong') {
    ...
    foreach ($rows as $r) {
        ...
        $item['value'] = round($r['total_score'] / 50, 2); //format(ROUND(total_score/50, 2),2)
        $item['LAN_DG'] = $num;
        $data[] = $item;
    }
    nv_jsonOutput($data);
...
if ($nv_Request->get_title('act', 'get,post', '') == 'get_thkhoaphong') {
    ...
    $st0 = "Select  count,from_date, to_date FROM " . TABLE . "_report_details_" . $get['year'] . " 
    ...
    $from_to = array();
    $r2 = 0;
    $rows = $db->query($st0)->fetchALL();
    for ($r = 0; $r < count($rows); $r++) {
        $r2 = $r + 1;
        $dscot = $dscot . '+IFNULL(l' . $r2 . ',0)';
        $tencot = $tencot . 'MAX(CASE WHEN count =' . $r2 . '  THEN format(ROUND(total_score/50, 2),2) END) AS l' . $r2 . ',';
    ...
    $st1 = "
        ...
        GROUP BY kp.tenkhoa, rp.account_report,  rp.arr_question
        ORDER BY kp.tenkhoa DESC";

    $count = $db->exec($st1);

    //Lấy dữ liệu trong bảng crosstable
    $st2 = "Select *," . $dscot . " as total   FROM " . TABLE . "_report_crosstable_" . $get['year'] . " 
      ORDER BY total DESC";
    // print($st);
    $ds = array();
