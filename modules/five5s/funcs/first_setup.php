...
if ($get['id_year'] > 0) {
	...
	(empty($get['view_id'])) ? $get['view_id'] = $get['id_year'] . "_" . "1" : '';

	//$get['view_id']=$get['id_year']."_"."1";//set mặc định lân đánh giá là 1 nếu ko có dữ liệu
	if (!empty($get['view_id'])) {

		//$get['view_id'] = $nv_Request->get_title('view_id', 'get,post', '');
		$arr = explode('_', $get['view_id']);
		$get['year'] = $arr[0];
		//(empty($get['count_evaluation'])) ? $get['count_evaluation'] = 1 : $get['count_evaluation'] = $arr[1]; //set mặc định lân đánh giá là 1 nếu ko có dữ liệu
		$get['count_evaluation'] = $arr[1];
	...
	$sql = "Select * FROM " . TABLE . "_report_" . $get['id_year'] . " where status = 1 order by id";
	$rows = $db->query($sql)->fetchAll();
	$tt = 0;
	$arr_kp = array();
	//thong tin tổng hợp theo lần đánh giá
	$tonghop = array();
	$tonghop['slkp'] = 0;
	$tonghop['sl_htdg'] = 0;
	$tonghop['sl_ddg'] = 0;
	...
	foreach ($rows as $row) {
		...
		for ($i = 0; $i < count($dskhoa); $i++) {
			...
			//var_dump($khoaphong['khoa']);
			//$arr_kp[$i]=$khoaphong['khoa'];
		}


		$array = $dskhoa;
		$itemsPerRow = 7; // Number of items per row
		(count($array) > 1) ? $totalItems = count($array) : $totalItems = 0;
		//$totalItems = count($array); // Total number of items in the array
		$totalRows = ceil($totalItems / $itemsPerRow); // Calculate the total number of rows
		$tbl = '<table>';
		$itemCount = 0;
		(empty($get['count_evaluation'])) ? $get['count_evaluation'] = 1 : '';
		$token = 'edit_evaluation&edit_id=' . $namapdung . '_' . $row['id'];
		$s = 'edit_evaluation&edit_id=2023_1&idd=tochuc_1';

		$func = 'edit_evaluation';
		$link =  MODULE_LINK . '&' . NV_OP_VARIABLE . '=' . $token;
		//$link = MODULE_LINK . '&' . NV_OP_VARIABLE . '=' . $func . '&idd=';

		for ($row_t = 1; $row_t <= $totalRows; $row_t++) {
			$tbl .= '<tr>';
			// Calculate the number of items for the current row
			$itemsInRow = ($row_t == $totalRows) ? ($totalItems % $itemsPerRow) : $itemsPerRow;

			for ($col = 1; $col <= $itemsInRow; $col++) {

				if ($itemCount < $totalItems) {
					//kiểm tra trạng thái đánh giá
					$st2 = "Select status,rating FROM " . TABLE . "_report_details_" . $get['id_year'] .
						" WHERE account_report ='" . $array[$itemCount] . "' AND count =" . $get['count_evaluation'] . " AND status > 0
					AND id_report=" . $row['id'];
					//$link= MODULE_LINK . '&' . NV_OP_VARIABLE . '=' . $func . '&view_id=' . $token.'&idd='.$array[$itemCount].'_'.$get['count_evaluation'];
					$link = MODULE_LINK . '&' . NV_OP_VARIABLE . '=' . $token . '&idd=' . $array[$itemCount] . '_' . $get['count_evaluation'];
					$r_status = $db->query($st2)->fetch();
					//print $r_status['status'];
					//tạo bảng khoa phòng theo các hàng đều nhau
					$tbl .= ' <td style="text-align:center;">';


					//lấy ra tỉ lệ xếp loại
					if (($row['status'] > 0) && ($row['id'] < 5)) {
						if ($r_status['rating'] == "Rất tốt") $tonghop['rattot']++;
						if ($r_status['rating'] == "Tốt") $tonghop['tot']++;
						if ($r_status['rating'] == "Trung bình") $tonghop['trungbinh']++;
						if ($r_status['rating'] == "Yếu") $tonghop['yeu']++;
					}


					if ($r_status['status'] == 1) {
						$tbl .= '<span style="margin-bottom: 5px;pa: 20px;" id="sp-kp" class="badge badge-warning mr-1"
					"> <a href="' . $link . '" style="color: white; font-size: 11px;">';
						if ($row['id'] < 5) $tonghop['sl_ddg']++;
					}
					if ($r_status['status'] == 2) {
						$tbl .= '<span style="margin-bottom: 5px;" id="sp-kp" class="badge badge-primary mr-1"
						"> <a href="' . $link . '" style="color: white; font-size: 11px;">';
						if ($row['id'] < 5) $tonghop['sl_htdg']++;
					}
					if ($r_status['status'] == 0) {
						$tbl .= '<span style="margin-bottom: 5px;" id="sp-kp" class="badge badge-success mr-1"
						"> <a href="' . $link . '" style="color: white; font-size: 11px;">';
						if ($row['id'] < 5) $tonghop['sl_cdg']++;
					}
		...
		$row['tbl_kp'] = $tbl;
		(count($array) > 1) ? $row['slkhoa'] = count($dskhoa) : $row['slkhoa'] = 0;
	...
	//var_dump($tonghop);
	//đưa vào bảng tổng hợp đánh giá
	$tonghop['slkp'] = $tonghop['sl_htdg'] + $tonghop['sl_ddg'] + $tonghop['sl_cdg'];

	//tính tỷ lệ %
	$tonghop['rattot'] = round(($tonghop['rattot'] / $tonghop['slkp']), 2) * 100;
	$tonghop['tot'] = round(($tonghop['tot'] / $tonghop['slkp']), 2) * 100;
	$tonghop['trungbinh'] = round(($tonghop['trungbinh'] / $tonghop['slkp']), 2) * 100;
	$tonghop['yeu'] = round(($tonghop['yeu'] / $tonghop['slkp']), 2) * 100;





	//report_content&view_id=2023&idd=1_e2fd7a8f12569237fa7f0d90e19627f8
	$xtpl->assign('TONGHOP', $tonghop);
	...
	foreach ($arr as $r) {
		...
		($r == $get['count_evaluation']) ? $ds['class'] = 'btn btn-primary' :'';	
...
(empty($get['count_evaluation'])) ? $get['count_evaluation'] = 1 : ''; //set mặc định lân đánh giá là 1 nếu ko có dữ liệu

$token = md5($client_info['session_id'] . $user_info['username']);
$func = "edit_evaluation&edit_id=" . $namapdung . '_' . $get['count_evaluation'];

//$ds['link_landanhgia'] =  MODULE_LINK . '&' . NV_OP_VARIABLE . '=' . $op . '&view_id=' . $token;
$link_dg =  MODULE_LINK . '&' . NV_OP_VARIABLE . '=' . $func . '&idd=';
//$link_dg = MODULE_LINK . '&' . NV_OP_VARIABLE . '=edit_evaluation&edit_id=' . $token;
$xtpl->assign('link_dg', $link_dg);
$xtpl->assign('TOKEN', $token);
$xtpl->assign('LANDG', $get['count_evaluation']);
//report_content&view_id=2023&idd=1_e2fd7a8f12569237fa7f0d90e19627f8
//đưa link vào nút xem báo cáo triển khai tổng hợp 5S
$func2 = "op=report_content&view_id=" . $namapdung;
$link_bcth = MODULE_LINK . '&' . $func2 . '&idd=' . $get['count_evaluation'] . '_' . $token;
$xtpl->assign('link_bcth', $link_bcth);
