...
foreach ($dskhoa as $ds) {
    ...
    foreach ($arr_op as $r) {
        ...
        ($r == $get['count_report'] && $get['account_view'] == $ds['account']) ? $ds2['class'] = 'btn btn-primary' : '';
...
if (!empty($get['idd']) && !empty($get['year'])) {
    ...
    //Gán vào nút đánh giá cho các lần đã khởi tạo, nhưng chưa đánh giá
    //check dữ liệu bảng kỳ đánh giá
    $st1 = "SELECT * FROM " . TABLE . "_evaluation_" .  $get['year'] . " Where 
    count_evaluation ="  . $get['count_report'] . " AND status >0";
    $ds1 = $db->query($st1)->fetch();
    //lay ra thong tin account_view trong bang ky danh gia
    if (!empty($ds1['count_evaluation'])) {
        $st2 = "SELECT id, view_report FROM " . TABLE . "_report_" .  $get['year'] . " ";
        //print($st2);
        $ds2 = $db->query($st2)->fetchALL();
        foreach ($ds2 as $dss) {
            $arr_acc = $dss['view_report'];
            //$arr_acc = explode(';',$arr_acc);
            if (strpos($arr_acc, $get['account_view']) !== false) {
                //echo $dss['id'] . '-' .  $arr_acc;
                $xtpl->assign('hien_bieumau', '');
                $xtpl->assign('hien_nutexcel', '');

                $func = 'edit_evaluation';
                $xtpl->assign('link_dg', MODULE_LINK . '&' . NV_OP_VARIABLE . '=' .
                    $func . '&edit_id=' . $get['year'] . '_' . $dss['id'] . '&idd=' . $get['idd']);
            } else {
                //$xtpl->assign('hien_bieumau', 'hidden');
                //$xtpl->assign('hien_nutexcel', 'hidden');
    ...
    if (!empty($ds['count'])) {
        ...
        $arr2['tenkhoabc'] = $get['account_view'];
