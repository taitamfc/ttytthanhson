...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM ' . NV_PREFIXLANG . '_' . $module_data . '_department WHERE id=' . $id;
    $row = $db->query($query)->fetch();
    if (isset($row['status']))     {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_department SET status=' . intval($status) . ' WHERE id=' . $id;
        $db->query($query);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('ajax_action', 'post')) {
    ...
    if ($new_vid > 0)     {
        ...
        {
            ++$weight;
            if ($weight == $new_vid) ++$weight;             $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_department SET weight=' . $weight . ' WHERE id=' . $row['id'];
            $db->query($sql);
        }
        $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_department SET weight=' . $new_vid . ' WHERE id=' . $id;
        $db->query($sql);
        $content = 'OK_' . $id;
    }
    $nv_Cache->delMod($module_name);
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $weight=0;
        $sql = 'SELECT weight FROM ' . NV_PREFIXLANG . '_' . $module_data . '_department WHERE id =' . $db->quote($id);
        $result = $db->query($sql);
        list($weight) = $result->fetch(3);
        
        $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_department  WHERE id = ' . $db->quote($id));
        $vote_id=$db->query('SELECT id FROM '.NV_PREFIXLANG.'_'.$module_data.'_vote where department_id = ' . $db->quote($id))->fetchColumn();
        ...
        $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_vote  WHERE department_id = ' . $db->quote($id));

        if ($weight > 0)         {
            $sql = 'SELECT id, weight FROM ' . NV_PREFIXLANG . '_' . $module_data . '_department WHERE weight >' . $weight;
            $result = $db->query($sql);
            while (list($id, $weight) = $result->fetch(3))
            {
                $weight--;
                $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_department SET weight=' . $weight . ' WHERE id=' . intval($id));
            }
        }
        $nv_Cache->delMod($module_name);
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Department', 'ID: ' . $id, $admin_info['userid']);
...
if ( !empty( $q ) ) {
    $where .= ' AND (name_department LIKE "%" "'.$q. '" "%")';
    $base_url .= '&q=' . $q;
}
if ( $status_ft>-1 ) {
...
$xtpl->assign('OP', $op);
$xtpl->assign('Q', $q);
$xtpl->assign('department_add',  NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=department_add');
if ( $ngay_tu > 0 )
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        if(empty($view['userid'])){
            $view['userid']='Chưa cập nhật';
        }else{
            $view['userid']=get_info_staff($view['userid'])['staff_code'].' - '.get_info_staff($view['userid'])['name_staff'];
        }
        ...
        $view['number_staff']=$db->query('SELECT count(*) FROM '.NV_PREFIXLANG.'_'.$module_name.'_department_staff where department_id='.$view['id'])->fetchColumn();
        $view['number_staff']=number_format($view['number_staff']);
        $view['link_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=department_add&amp;id=' . $view['id'];
        $view['link_staff'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=department_staff&amp;department_id=' . $view['id'];
