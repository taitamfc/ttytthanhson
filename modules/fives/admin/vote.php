...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM ' . NV_PREFIXLANG . '_' . $module_data . '_vote WHERE id=' . $id;
    $row = $db->query($query)->fetch();
    if (isset($row['status']))     {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_vote SET status=' . intval($status) . ' WHERE id=' . $id;
        $db->query($query);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('ajax_action', 'post')) {
    ...
    $content = 'NO_' . $id;
    if ($new_vid > 0)     {
        $sql = 'SELECT id FROM ' . NV_PREFIXLANG . '_' . $module_data . '_vote WHERE id!=' . $id . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch())
        {
            ++$weight;
            if ($weight == $new_vid) ++$weight;             $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_vote SET weight=' . $weight . ' WHERE id=' . $row['id'];
            $db->query($sql);
        }
        $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_vote SET weight=' . $new_vid . ' WHERE id=' . $id;
        $db->query($sql);
        $content = 'OK_' . $id;
    }
    $nv_Cache->delMod($module_name);
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_vote  WHERE id = ' . $db->quote($id));
        $vote_id=$db->query('SELECT id FROM '.NV_PREFIXLANG.'_'.$module_data.'_vote_vote where vote_id = ' . $db->quote($id))->fetchColumn();
        $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_vote_item  WHERE vote_vote_id = ' . $db->quote($vote_id));
        $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_vote_vote  WHERE vote_id = ' . $db->quote($id));
        $nv_Cache->delMod($module_name);
...
$row_id = $nv_Request->get_string( 'row_id', 'get,post', 0);
...
if(!empty($time))
{$xtpl->assign('TIME', date('d/m/Y',$time));
$xtpl->assign('khoa', 'disabled="disabled"');

}
else{$xtpl->assign('TIME', date('d/m/Y',NV_CURRENTTIME));
$xtpl->assign('khoa', '');}

if($row_id >0)
{
  $xtpl->assign('title', get_row_id($row_id)['title']);
  $xtpl->assign('row_id', $row_id);  
  $xtpl->parse('add.row_id');
  
}

if($userid >0)
{
  $xtpl->assign('title', get_info_user($userid)['username']);
  $xtpl->assign('userid', $userid);  
  $xtpl->parse('add.userid');
  
}
if($department_id >0)
{
  $xtpl->assign('title', get_info_department($department_id)['name_department']);
  $xtpl->assign('department_id', $department_id);  
  $xtpl->parse('add.department_id');
...
if (!$nv_Request->isset_request('id', 'post,get')) {
    ...
    $db->sqlreset()
    ->select('COUNT(*)')
    ->from('' . NV_PREFIXLANG . '_' . $module_data.'_vote')
    ->where('1=1'.$where);
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        $tongtrungbinh=$db->query('SELECT avg(trungbinh) FROM '.NV_PREFIXLANG.'_'.$module_data.'_vote_vote where  vote_id='.$view['id'])->fetchColumn();
        $view['trungbinh']=number_format($tongtrungbinh,2);
        $view['stt']=$i;
        $view['title_list']= get_info_vote($view['row_id'])['title'];
        $view['tungay']= date('d/m/Y',get_info_vote($view['row_id'])['time_from']);
        $view['denngay']= date('d/m/Y',get_info_vote($view['row_id'])['time_to']);
        $view['department_name']= get_info_department($view['department_id'])['name_department'];
        $view['username']=get_info_user($view['userid'])['first_name'];
        $view['time']=date('d/m/Y H:i',$view['time_add']);
        $view['link_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&action=add&amp;id=' . $view['id'].'&amp;time='.$view['time_add'].'&amp;row_id='.$view['row_id'].'&amp;userid='.$view['userid'].'&amp;department_id='.$view['department_id'].'';
