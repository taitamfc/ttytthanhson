...
if (! empty($del_array)) {
    ...
    foreach ($del_array as $id) {
        list($id, $title, $urlimg) = $db->query('SELECT id, hoten, dienthoai  FROM ' . NV_PREFIXLANG . '_' . $module_data . '_dangky WHERE id=' . $id)->fetch(3);
        if ($id > 0) {
            nv_insert_logs(NV_LANG_DATA, $module_name, 'log_del_rows', $title, $admin_info['userid']);

            $db->exec('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_dangky WHERE id=' . $id);
