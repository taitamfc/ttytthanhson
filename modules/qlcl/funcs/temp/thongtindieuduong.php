...
if (empty($user_info)){	$url = MODULE_LINK . '&' . NV_OP_VARIABLE . '=login';nv_redirect_location($url); exit();}
	...
	$xtpl = new XTemplate('frm_thongtindieuduong.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_info['module_theme']);
	...
	$id_khoaphong =$nv_Request->get_int('find_id', 'get,post',0); 
	//check tài khoản khoa 
	$tk_khoa=0;$cri_khoa='';
	$tk_khoa=check_khoaphong($user_info['username']);if ($tk_khoa>0) {$id_khoaphong =$tk_khoa;$cri_khoa= ' and (id=' .$id_khoaphong .')';}
	...
	//check ds tang cuong
	$sql = 'SELECT * FROM ' . TABLE . "_yeucaunhanluc WHERE status = 1 and user_yeucau like '".$user_info['username']."' and tg_ngayketthuc>=".intval(NV_CURRENTTIME);
