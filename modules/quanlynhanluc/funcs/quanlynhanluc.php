...
if (empty($user_info)){	$url = MODULE_LINK . '&' . NV_OP_VARIABLE . '=login';nv_redirect_location($url); exit();}
	...
	if ($sta=='nhucaunhanluc_step1')	
	{
		...
		$err=0;$tb='';
		if ($data['tg_tungay']<$today 
			or $data['tg_denngay']<$today 
			or $data['tg_tungay']>$data['tg_denngay'])
			$tb=++$err.'.Lựa chọn thời gian nhu cầu không hợp lệ, vui lòng xem lại!';
		...
		if ($row_id>0)
		{
			...
			$sql="SELECT * from ".TABLE. "_groupuser where id_nhomquyen=102";
		...
		{
			$ketqua['status']='err';$ketqua['mess']=$tb;
		}
	...
	if ($sta=='huypheduyet')	
	{
		...
		if(empty($data['ghichu_duyet']))
			$ketqua='ERR_Lý do từ chối '.$lang_module['error_empty'];
		else 
		{
				...
				$row_id=$stmt->execute();
				if ($row_id>0) $ketqua='OK_Đã từ chối phê duyệt thành công!';
				else $ketqua='ERR_Lệnh cập nhật từ chối không thực hiện được!';
			//Update notification
				$sql = 'Update ' . TABLE. "_notification SET step=0, viewed=0 WHERE code_pro like '".$data['code_pro']."'";
	...
	if ($sta=='ketthucdieudong')	
	{
		...
		else */
		{
				...
				WHERE code_pro like '".$data['code_pro']."'";
				$stmt = $db->prepare($sql);
				//$stmt->bindParam(':ghichu_yeucau', date('d/m/y h:i:s'), PDO::PARAM_STR);
				$row_id=$stmt->execute();
				if ($row_id>0) 
				{ 
					$ketqua='OK_Đã kết thúc điều động thành công!';
					//Update notification
					$sql = 'Update ' . TABLE. "_notification SET step=4, viewed=0 WHERE code_pro like '".$data['code_pro']."'";
					$stmt = $db->prepare($sql)->execute();
					//Update dscanbo
					$sql = 'Update ' . TABLE. "_canbo set tangcuong_khoa='', tangcuong_tungay=0, tangcuong_denngay=0
					WHERE id in (select id_canbo from ".TABLE. "_canbodieudong where code_pro like '".$data['code_pro']."')";
					$stmt = $db->prepare($sql)->execute();
					// có nên cập nhật table canbodieudong theo ngày thực tế kết thúc ko?
	...
	if ($sta=='nhucaunhanluc_step2')	
	{
		...
		if ($data['tg_ngaybatdau']<$today  
			or $data['tg_ngayketthuc']<$today  
			or $data['tg_ngaybatdau']>$data['tg_ngayketthuc'])
			$mes=++$err.'.Lựa chọn thời gian điều động không hợp lệ, vui lòng xem lại!';
		if ($err==0)
		{
