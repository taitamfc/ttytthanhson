...
($rs[0]['id_report'] != 5) ? $num = ['I.', 'II.', 'III.', 'IV.', 'V.', 'VI.', 'VII.', 'VIII.'] : $num = array();
...
if (isset($post['submit'])) {
    ...
    if ($update) {
        ...
        arr_question, recommendation,limitation,bonus,discipline,count,account_report,id_report,name_report,account_input, content,status
        )
        VALUES (:images ,:evaluation_type,:arr_question_type,:arr_result_question,:total_score, :rating,:arr_team, :from_date,:to_date,:created_date,
            :arr_question, :recommendation, :bonus, :discipline, :limitation,:count,:account_report,:id_report,:name_report,:account_input,:content,:status
        )";
        //         print($st);
        $stmt = $db->prepare($st);
        $stmt->bindParam(':evaluation_type', $post['evaluation_type']);
