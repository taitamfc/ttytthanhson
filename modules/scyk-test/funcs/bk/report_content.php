...
if (count($ds) > 0) {
    ...
    foreach ($ds as $row) {
        ...
        ($get['count_report'] ==  $row['count_evaluation']) ? $row['class'] = 'btn btn-primary' : $row['class'] = 'btn btn-success btn-outline-success';
...
if (!empty($get['idd']) && !empty($get['count_report'])) {
 ...
 WHERE status >0 and count_evaluation=" . $get['count_report'] . " order by count_evaluation asc";
    ...
    //$tongdtb = $row['count(id)'];
    //$avgch = round($avg *intval($row['count(id)']) /intval($row['count(id)'])*50,2);

    $avgch = $avg * $row['count(id)'];
    $avgch2 = 50 * $row['count(id)'];
    ($avgch2 ==0 ? $avgch2 =1:'');
    $avgch = round($avgch /  $avgch2, 2);

    //echo 'avg='.$avgch.'---'. $avgch2;
    ...
    for ($r = 0; $r < count($rows); $r++) {
        ...
        if (empty($d1) || empty($d2))
            ...
            $from_to['ngay'] = $d1 . ' : ' . $d2;
        $row['stt'] = $r2;
        //($r2<count($rows))?$row['stt'] = $r2 : '';

        $xtpl->assign('LAN', $row['stt']);
        $xtpl->parse('main.LAN');
