...
if ($nv_Request->get_title('act', 'get,post', '') == 'hienmodal') {
	...
	if ($act = $nv_Request->get_title('sta3', 'get,post', '')) {
		...
		if ($dateTime !== false) {
			...
			$data['ngaygiott'] = $formattedDate;
			//echo 'MASOBC: '.$data_insert['masobc_a'].'---' . $formattedDate;
		} else {
			$data['ngaygiott'] = date("Y-m-d H:i:s", time());
...
and masobc_a ='$num'";
	...
	//hiển thị thêm câu 2 -12 trong (Bảng B)
	...
	AND (numv BETWEEN 2 AND 13) OR (numv BETWEEN 16 AND 21)
    ...
    ";
		...
		foreach ($rows as $row) {
			...
			$xtpl->parse('main.NHOMCAUHOI2B.CAUHOI2B');

			//echo $row['type_control'].'|';
		}
		$xtpl->parse('main.NHOMCAUHOI2B');
		$xtpl->parse('main.DS2B');
		//echo '<br>';
	}
...
and masobc_a ='$num'";
	...
	foreach ($groups as $group) {
		...
		foreach ($rows as $row) {
			...
			$xtpl->parse('main.NHOMCAUHOI1A.CAUHOI1A');

			//echo $row['type_control'].'|';
		}
		$xtpl->parse('main.NHOMCAUHOI1A');
		$xtpl->parse('main.DS');
		//echo '<br>';
	}
