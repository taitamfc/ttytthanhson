...
if (isset($post['submit'])) {
    ...
    //lấy các giá trị tên control trong bảng voting_row_b
    //$post['test'] = $nv_Request->get_array('qtkt[]', 'post');
    //var_dump( $post['test']);
    ...
    //insert to db
    /* cách 1
    $stt = "INSERT INTO " . TABLE . "_voted_result_b " . $tencot . ' ' . $values;
       echo '<br>'.$stt;
    $stmt = $db->prepare($stt);
    //var_dump( $stmt);
    foreach ($rows as $row) {
    ...
    */
    ...
    //cách 2
    $st = "INSERT INTO " . TABLE . "_voted_result_b " . $tencot . ' ' . $values;
    echo $st;
    var_dump($data_insert);
    $row_id = $db->insert_id($st, 'id', $data_insert);
    //echo 'hangcuoi=' . $row_id;
    $ketqua['status'] = ($row_id > 0) ? 'OK' : 'ERR';
    echo '<br>' . $ketqua['status'];
    ...
    // Ví dụ sử dụng
    $currentMaso = "10";
    $newMaso = generateMaso($currentMaso);
    echo '<br>'.$newMaso; // In ra "MSBC002"
