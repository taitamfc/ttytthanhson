		...
		for ($row_t = 1; $row_t <= $totalRows; $row_t++) {
			...
			for ($col = 1; $col <= $itemsInRow; $col++) {
				...
				if ($itemCount < $totalItems) {
					...
					$st2 = "Select status FROM " . TABLE . "_report_details_" . $get['id_year'] .
					...
					if ($r_status['status'] == 0) $tbl .= '<span id="sp-kp" class="badge badge-info mr-1"
					onclick="get_khoaphong_lan(this);">';
					if ($r_status['status'] == 1) $tbl .= '<span id="sp-kp" class="badge badge-success mr-1"
					onclick="get_khoaphong_lan(this);">';
					if ($r_status['status'] == 2) $tbl .= '<span id="sp-kp" class="badge badge-warning mr-1"
					onclick="get_khoaphong_lan(this);">';
