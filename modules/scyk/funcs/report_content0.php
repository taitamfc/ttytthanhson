...
if ($ds) {
    ...
    // print('Hien danh gia');
    foreach ($ds as $row) {
        $token = $get['year'] . '_' . '&idd=' . $row['count_evaluation'] . '_' . md5($client_info['session_id'] . $user_info['username']);
...
if (!empty($get['idd'])) {
    ...
    $st = "Select  from_date, to_date, count FROM " . TABLE . "_report_details_" . $get['year'] . " 
