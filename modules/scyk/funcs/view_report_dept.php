...
$today->modify('-1 week'); //before 1 week

// Calculate the timestamp of the first day (Monday) of the current week
$firstDayOfWeekTimestamp = strtotime("monday this week", $today->getTimestamp());

// Calculate the timestamp of the last day (Sunday) of the current week
$lastDayOfWeekTimestamp = strtotime("sunday this week", $today->getTimestamp());

// Create DateTime objects from the calculated timestamps
$firstDayOfWeek = new DateTime();
$firstDayOfWeek->setTimestamp($firstDayOfWeekTimestamp);

$lastDayOfWeek = new DateTime();
$lastDayOfWeek->setTimestamp($lastDayOfWeekTimestamp);

$tg_tungay = $firstDayOfWeek->format('d-m-Y');
$tg_denngay = $lastDayOfWeek->format('d-m-Y');
...
$fromdate = $firstDayOfWeek->format('Y-m-d');
$todate = $lastDayOfWeek->format('Y-m-d');

$xtpl->assign('hienbieumau', 'hidden');
hien_bieumau($fromdate,$todate);
...
function hien_bieumau($tg_tungay, $tg_denngay){
    ...
    //echo "To Date value: " . htmlspecialchars($tg_tungay);

    $st = "SELECT masobc_a,kpbc,mota,thoigian,status,note FROM " . TABLE . "_voted_result_a
	WHERE DATE(thoigian) BETWEEN " . "'" . $tg_tungay . "'" . " and " . "'" . $tg_denngay . "'" . " 
