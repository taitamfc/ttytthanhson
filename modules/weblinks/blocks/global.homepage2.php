...
if (!nv_function_exists('nv_slide_homepage2')) {
    /**
     ...
     * @return string
     */
    function nv_block_config_page_list2($module, $data_block, $lang_block)
    {
     ...
     * @return array
     */
    function nv_block_config_page_list2_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = [];
        $return['error'] = [];
     ...
     * @return string
     */
    function nv_slide_homepage2($block_config)
    {
        ...
        $db->sqlreset()->select('r.*, c.title as tennhom')->from(NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'].'_rows r inner join '
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/weblinks/block_homepage2.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/weblinks/block_homepage2.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block_homepage2.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/weblinks' );
			$xtpl->assign('THEMES_URL', NV_BASE_SITEURL. '/themes/' . $block_theme.'/css/slide');
			$xtpl->assign('detail', $lang_module['detail']);
			$xtpl->assign('url_icon', '/themes/' . $module_info['template'] . '/images');
			$stt=0;
            foreach ($list as $l) {
...
if (defined('NV_SYSTEM')) {
    $content = nv_slide_homepage2($block_config);
}
