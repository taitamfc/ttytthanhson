...
if (!nv_function_exists('nv_slide_homepage3')) {
    /**
     ...
     * @return string
     */
    function nv_block_config_page3_list($module, $data_block, $lang_block)
    {
     ...
     * @return array
     */
    function nv_block_config_page3_list_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = [];
        $return['error'] = [];
     ...
     * @return string
     */
    function nv_slide_homepage3($block_config)
    {
        ...
        $db->sqlreset()->select('r.*, c.title as tennhom')->from(NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'].'_rows r left join '
		.NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'].'_cat c on r.catid=c.catid ')
		->where('r.status = 1 and r.catid='.$block_config['catid'] )->order('r.edit_time DESC')->limit($block_config['numrow']);
        $list = $nv_Cache->db($db->sql(), 'id', $module);

        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/weblinks/block_homepage3.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/weblinks/block_homepage3.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block_homepage3.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/weblinks' );
			$xtpl->assign('THEMES_URL', NV_BASE_SITEURL. '/themes/' . $block_theme.'/css/slide');
			$xtpl->assign('detail', $lang_module['detail']);
			$xtpl->assign('url_icon', NV_BASE_SITEURL . '/themes/' . $module_info['template'] . '/images');
			$stt=0;
            foreach ($list as $l) {
...
if (defined('NV_SYSTEM')) {
    $content = nv_slide_homepage3($block_config);
}
