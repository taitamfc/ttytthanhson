...
if (!nv_function_exists('nv_slide_homepage4')) {
    /**
     ...
     * @return string
     */
    function nv_block_config_page4_list($module, $data_block, $lang_block)
    {
     ...
     * @return array
     */
    function nv_block_config_page4_list_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = [];
        $return['error'] = [];
     ...
     * @return string
     */
    function nv_slide_homepage4($block_config)
    {
		...
		->where('r.status = 1 and r.catid='.$block_config['catid'] )->order('r.add_time DESC')->limit($block_config['numrow']);
        $list = $nv_Cache->db($db->sql(), 'id', $module);

        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/weblinks/block_homepage4.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/weblinks/block_homepage4.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block_homepage4.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/weblinks' );
...
if (defined('NV_SYSTEM')) {
    $content = nv_slide_homepage4($block_config);
}
