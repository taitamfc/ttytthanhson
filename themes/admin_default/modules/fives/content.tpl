...
<div class="panel panel-default">
    <div class="panel-heading">
        <h3 class="panel-title" style="float:left"><i class="fa fa-list"></i> {LANG.content_list}</h3> 
                    ...
                    <div class="form-group">
                        <div class="input-group" style="width:100%">
                          ...
                          {LANG.cats}
                            ...
                            <select class="form-control catsid" name="catsid">
                                <!-- BEGIN: chon -->
                                <option value="0">Vui lòng chọn loại đánh giá</option>
                                <!-- END: chon -->
                                <option value="{catsid}">{cats_title}</option>
                             </select>
                        </div>
                    </div>
                </div>
        ...
        <table class="table table-striped table-bordered table-hover">
            <thead>
               <tr>
                    ...
                    <th class=" text-center">{LANG.cats}</th>
...
<script type="text/javascript">
      $('.catsid').select2({
        placeholder:"Vui lòng chọn loại đánh giá", 
        ajax: {
            url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=ajax&mod=get_cats',
            dataType: 'json',
            delay: 250,
...
<form class="form-horizontal" action="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&amp;{NV_NAME_VARIABLE}={MODULE_NAME}&amp;{NV_OP_VARIABLE}={OP}&action=add" method="post">
    <input type="hidden" name="id" value="{ROW.id}" />
       <div class="form-group">
        <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.cats}</strong> <span class="red">(*)</span></label>
        <div class="col-sm-19 col-md-20">
             <select class="form-control required catsid" name="catsid">
                <!-- BEGIN: chon -->
                <option value="0">Vui lòng chọn loại đánh giá</option>
                <!-- END: chon -->
                <option value="{ROW.catsid}">{ROW.cats_title}</option>
             </select>
        </div>
    </div>
    <div class="form-group">
