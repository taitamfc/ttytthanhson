      ...
      <div class="card">
        <div class="card-block">
          ...
          <form class="md-float-material" action="{link_frm}" method="post" id="myform" enctype="multipart/form-data">
            ...
            <input type="hidden" id="id_sta" name="txt_sta" value="{sta}" />
            <input type="hidden" id="id_tm_kp" name="txt_tm_kp" value="{tm_kp}" />
            <div class="">
              <table class="table">
                <tbody>
                  <tr>
                    <td colspan="2">
                      <div class="input-group">
                        ...
                        <select id="account" name="cbo_account" class="form-control" placeholder="Chọn Khoa Phòng"
                          title="" data-toggle="tooltip" data-original-title="Khoa Phòng Đánh Giá"
                          onchange="change_account(this)">
        ...
        <tr style="background-color: #e8edea;border-color: #73b4ff;outline: 1px dashed #fff;outline-offset: -5px;">
          ...
          <td class="text-center col-2">
            <input id="end-date" name="to_date" value="{thongtin.to_date}" type="text"
              class="dataValue form-control bg-success" readonly disabled />
          </td>
        ...
        <tr>
          ...
          <td class="align-middle text-left col-2">
            <b><i class="fa fa-hand-o-right" aria-hidden="true"></i> Trạng thái đánh giá: <span
                class="label {thongtinchung.class}">{thongtinchung.trangthai}</span></b>
          </td>
        </tr>
          ...
          <thead>
            <tr>
              ...
              <th class="text-center header" colspan="2">
                DTB: <input class="dtb btn btn-out btn-warning btn-square" type="text" name="dtb" id="id_dtb"
            ...
            <th class="m0" style="text-align: center;">{tieumuc.m0} </th>
            <th class="m1" style="text-align: center;">{tieumuc.m1} </th>
            <th class="m2" style="text-align: center;">{tieumuc.m2} </th>
            <th class="m" style="text-align: center;">{tieumuc.tongso} </th>
        ...
        <div class="accordion-block color-accordion-block">
          <div class="color-accordion ui-accordion ui-widget ui-helper-reset" id="color-accordion" role="tablist">
            ...
            <a class="accordion-msg b-none ui-corner-top ui-state-default ui-accordion-icons scale_active ui-accordion-header-collapsed ui-corner-all"
              ...
              <strong>{group.stt} {group.name_question_type} </strong><span id="{group.stt}"
                class="{group.class}">{group.ctl}</span></a>
                    ...
                    <td>
                      <div class="ks-cboxtags">
                        <input class="calc form-check-input" type="radio" id="r{score.id_question}_{score.score}"
                          name="a[{score.id_question}]" value="{score.score}-{score.id_group}" {score.checked}
                          onchange="updateSpan('{score.id_group}', this)">
            ...
            <div class="accordion-desc ui-accordion-content ui-corner-bottom ui-helper-reset ui-widget-content"
              ...
              <table id="myTable" class="cell-border table table-hover" style="width:100%">
                <thead>
                  ...
                  <tr>
                    <td><textarea id="bonus" name="name_bonus" rows="5" class="form-control">{thongtin.bonus}</textarea>
                    </td>
                  </tr>
...
<!-- xu ly button radion so cau hoi con lai theo span -->
<script>
  function updateSpan(spanId, radio) {
    if (radio.checked) {

      var spanElement = document.getElementById(spanId);
      var spanText = parseInt(spanElement.textContent) - 1;
      if (spanElement) {
        if (spanText > 0) {
...
<script>
  $(function() {
    ...
    $('#myform').on('submit', function(e) {
      ...
      //check currentdate
      var currentDate = $('#current-date').datepicker('getDate');
      var startDate = $('#start-date').datepicker('getDate');
      var endDate = $('#end-date').datepicker('getDate');

      if (currentDate !== null && startDate !== null && endDate !== null) {
        if (!(currentDate >= startDate && currentDate <= endDate)) {
          alert('Lưu ý!!! Ngày đánh giá không hợp lệ ( từ ngày >= ngày đánh giá <= đến ngày)');
          e.preventDefault();
        }
      } else {
        alert('Lưu ý !!! Bạn Chưa chọn ngày đánh giá ');
        e.preventDefault();
      }

      var mValue = $(".m").text();
      //alert(mValue);
      var id_rp='{id_rp_detail}';
      if (id_rp === '5' && mValue < 40) {
        //alert('m='+mValue);
        alert('Lưu ý !!! Bạn chưa hoàn thành các tiểu mục đánh giá ');
        e.preventDefault();
      } else if (id_rp !== '5' && mValue < 50) {
        alert('Lưu ý !!! Bạn chưa hoàn thành các tiểu mục đánh giá ');
        e.preventDefault();
      } else {
...
<script>
  function calcscore() {
    ...
    let m = score_0 + score_1 + score_2;
    let dtb = (score / (score_0 + score_1 + score_2)).toFixed(2);
    ...
    $(".m1").text(score_1);
    $(".m2").text(score_2);
    var id_sta_text = [score_0, score_1, score_2].join(",");
    $("#id_sta").val(id_sta_text);
