					...
					<div class="table-responsive" style="padding-bottom: 100px;">
						...
						<table id="tbl_danhsach" class="table table-hover" style="width:100%">
							...
							<tbody>
								<!-- BEGIN: DSBC -->
								<tr class="vl">
									<th style="vertical-align: middle;">{DSBC.stt}</th>
									<td style="vertical-align: middle;">{DSBC.masobc}</td>
									<td style="vertical-align: middle;">{DSBC.khoaphong}</td>
									<td style="vertical-align: middle; white-space:pre-wrap; word-break:break-word">{DSBC.tomtatnd}
									</td>

									<td style="vertical-align: middle;">{DSBC.ngaygio}</td>
									<td style="vertical-align: middle;">
									<label class="label {DSBC.class}"><strong>{DSBC.trangthai}</strong></label>
									</td>
									<td style="vertical-align: middle;">
										<a style="color: #fff;" href="#" title="Xem /In BC" onclick="view_info(this);"
											class="btn btn-mini btn-success btn-round">
											<i class="fa fa-eye" aria-hidden="true"></i> </a>
										<a style="color: #fff;" href="#" title="Xóa BC" onclick="del_info(this);"
											class="btn btn-mini btn-danger btn-round">
...
<script>
	...
	$(document).ready(function() {
		...
		var act = getParameterValue("act");

		// Check if 'msbcValue' is null
		if (msbcValue === null) {

		} else {
			//kiểm tra bấm nút xóa thì ko thiển thị modal
			if (act != "deletebc") {
				$("#mymodal").modal("show");
			}
	...
	function del_info(button) {
		...
		//tomtatnd + "\nNgaygio: " + ngaygio + "\nTrangthai: " + trangthai;
		var url = "{link_frm}";
		if (confirm("Bạn có chắc chắn xóa báo cáo mã số: " + masobc + " này?")) {
			...
			$.ajax({
					type: 'post',
					cache: !1,
					url: url + '&act=deletebc&msbc=' + masobc,
					dataType: "text",
					success: function(result) {						
						location.reload();				
						},
